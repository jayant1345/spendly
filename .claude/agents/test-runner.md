...
- **Relevant project constraint**: Flag if the failure relates to known project rules (e.g., raw SQL f-strings instead of `?` placeholders, hardcoded URLs instead of `url_for()`, missing `PRAGMA foreign_keys = ON`)

### 3. Warning Flags
- Identify any test output that suggests architecture violations even if tests pass
...
Always check test output for signals of these common Flask project mistakes:
...
- If a test file exercises a stub route that is not yet implemented, flag this clearly: "This test targets a stub route — implementation must precede testing"
